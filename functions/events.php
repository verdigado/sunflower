...
function germanDate2intDate($germanDate)
{
    ...
    [$day, $month, $year, $hours, $minutes] = preg_split('/[^0-9]/', (string) $germanDate);
...
function intDate2germanDate($intDate)
{
    ...
    [$year, $month, $day, $hours, $minutes] = preg_split('/[^0-9]/', (string) $intDate);
...
function sunflower_prepare_event_time_data($post)
{
    ...
    $event_more_days = ($_sunflower_event_until && date('jFY', $_sunflower_event_from) !== date('jFY', $_sunflower_event_until));
    ...
    $time = false;
    if (date('H:i', $_sunflower_event_from) !== '00:00' && ! $_sunflower_event_whole_day) {
        $time = sprintf(
            '%s %s',
