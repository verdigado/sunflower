(function (wp) {
  ...
  const withIconControl = wp.compose.createHigherOrderComponent((BlockEdit) => (props) => {
    const { attributes, setAttributes, name } = props;
    if (name !== 'core/button') return createElement(BlockEdit, props);
    ...
    return createElement(wp.element.Fragment, {},
      ...
      createElement(InspectorControls, {},
        createElement(PanelBody, { title: 'Icon auswählen', initialOpen: false },
          createElement(SelectControl, {
            ...
            onChange: (val) => setAttributes({ myIcon: val }),
          })
        )
      )
    );
  ...
  addFilter('editor.BlockEdit', 'sunflower/button-icon-control', withIconControl);
  ...
  (element, blockType, attributes) => {
    ...
    if (linkElement && linkElement.props && typeof linkElement.props.className === 'string') {
      // Entferne FontAwesome-Klassen vom Wrapper
      element.props.className = wrapperClass
        .split(' ')
        .filter(c => !c.startsWith('fa-') && !c.startsWith('fas'))
        .join(' ');
